...
def chat_with_deepseek(messages, model_name, max_retries=5, timeout=60):
    ...
    # Prepare the DeepSeek API payload - follows the standard chat completions format
    data = {
        "model": model_name,
        "messages": messages,
        "max_tokens": 8192,
    ...
    # For debugging - log first message (truncated)
    first_msg = messages[0]["content"]
    logger.debug(f"First message (truncated): {first_msg[:100]}...")
    ...
    for attempt in range(max_retries):
        try:
            ...
            # If we get a bad request, try to extract the error message
            if response.status_code == 400:
                ...
                # Check if we can identify specific issues
                error_msg = str(error_content)
                if "model" in error_msg and "not found" in error_msg.lower():
                    logger.error(f"The model '{model_name}' was not found. Available models may include: deepseek-chat, deepseek-coder")
                    if attempt == 0:  # Only on first attempt
                        # Try with a different model as fallback - deepseek-chat is often available
            ...
            # Try to get response body for more info on the error
            try:
...
def test_model_availability(models_to_test, timeout=30):
    ...
    # Return any available models
    available_models = [model for model, status in results if "Available" in status]
    return available_models

def main():
    parser = argparse.ArgumentParser(description='Analyze C, C++, and Python code files with comprehensive, step-by-step explanations using DeepSeek API')
    ...
    if args.list_models:
        ...
        print("")
        print("Note: Model availability may vary. If unsure, use 'deepseek-coder'")
    ...
    if args.check_models:
        models_to_test = [
            ...
            "deepseek-lite",
            "deepseek-ai/deepseek-coder-7b-instruct",  # Try fully qualified names too
