...
# Script to build and run the MNIST Docker project

# Create necessary directories
mkdir -p mnist_docker_project
cd mnist_docker_project

# Copy original MNIST code
cp ../Algorithms/AI/MachineLearning/Deep_learning/MINIST_WRITING_Python/mnist_recognition.py .

# Check and copy model files
if [ -f "../Algorithms/AI/MachineLearning/Deep_learning/MINIST_WRITING_Python/best_mnist_model.pth" ]; then
    cp ../Algorithms/AI/MachineLearning/Deep_learning/MINIST_WRITING_Python/best_mnist_model.pth .
fi

if [ -f "../Algorithms/AI/MachineLearning/Deep_learning/MINIST_WRITING_Python/mnist_model.pth" ]; then
    cp ../Algorithms/AI/MachineLearning/Deep_learning/MINIST_WRITING_Python/mnist_model.pth .
fi

# Copy test images
cp ../Algorithms/AI/MachineLearning/Deep_learning/MINIST_WRITING_Python/*.png .
...
# Set environment variables - disable CUDA (run in CPU mode)
ENV CUDA_VISIBLE_DEVICES=""

# Image processing mode (train or inference)
ENV MODE="inference"
ENV IMAGE_PATH="5.png"
...
services:
  ...
  mnist-inference:
    build:
      ...
      dockerfile: Dockerfile
    image: mnist-model:latest
    container_name: mnist-inference
    environment:
      - MODE=inference
      - IMAGE_PATH=5.png
      - CUDA_VISIBLE_DEVICES=""
...
This project is an example of running an MNIST digit recognition model in a Docker container.

## Project Structure

```
mnist_docker_project/
├── Dockerfile            # Docker image build file
├── requirements.txt      # Python dependency file
├── entrypoint.sh         # Container entrypoint script
├── mnist_recognition.py  # Original training code
├── mnist_inference.py    # Simplified inference-only script
...
```bash
# Run inference on default image (5.png)
docker run --rm -v $(pwd):/app mnist-model:latest

# Run inference on another image
docker run --rm -v $(pwd):/app -e IMAGE_PATH=3.png mnist-model:latest
```
...
docker-compose run mnist-inference

# Run in training mode (not yet implemented)
# docker-compose run mnist-train
```

## Results

Running inference will generate the following files:
- `original_[image_name].png`: Original image
- `preprocessed_[image_name].png`: Preprocessed image variants
- `prediction_[image_name].png` Or `comparison_[image_name].png`: Prediction results

## Notes

- Training mode is not yet fully implemented.
- To enable GPU support, you need to modify the Dockerfile and docker-compose.yml.
EOF

echo "All files have been created. You can now run the MNIST model using Docker."
echo "Run the following commands to get started:"
echo "  cd mnist_docker_project"
echo "  docker build -t mnist-model:latest ."
echo "  docker run --rm -v \$(pwd):/app mnist-model:latest"