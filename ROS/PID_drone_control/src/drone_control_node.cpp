...
int main(int argc, char** argv) {
    ...
    const double move_step = 0.3; // Distance to move with each key press
    while (ros::ok()) {
        std::string input;
        if (std::getline(std::cin, input)) { // Blocking call for input
            ...
            } else if (input.size() == 1) { // Single character for movement
                if (!position_received) {
                    ROS_ERROR("No position data yet, cannot move relative to current position.");
                } else {
                    // Set target relative to current position
                    target_pos = current_pos; // Start from current position
                    if (input == "a") { // Move left (decrease y)
                        target_pos.y -= move_step;
                        ROS_INFO("Moving left to: (%.2f, %.2f, %.2f)", target_pos.x, target_pos.y, target_pos.z);
                    } else if (input == "d") { // Move right (increase y)
                        target_pos.y += move_step;
                        ROS_INFO("Moving right to: (%.2f, %.2f, %.2f)", target_pos.x, target_pos.y, target_pos.z);
                    } else if (input == "s") { // Move down (decrease z)
                        target_pos.z -= move_step;
                        ROS_INFO("Moving down to: (%.2f, %.2f, %.2f)", target_pos.x, target_pos.y, target_pos.z);
                    } else if (input == "w") { // Move up (increase z)
                        target_pos.z += move_step;
                        ROS_INFO("Moving up to: (%.2f, %.2f, %.2f)", target_pos.x, target_pos.y, target_pos.z);
                    } else {
                        ROS_ERROR("Invalid key: Use 'a', 'd', 's', or 'w'.");
                        continue; // Skip starting the PID thread
                    }
                    target_set = true;
                    pid_thread = std::thread(pidControlThread);
                }
            } else {
                ROS_ERROR("Invalid input: Use 3 numbers (position), 6 numbers (velocity), 9 numbers (gains), or 'a/d/s/w'.");
            }
        }
    }
    // Cleanup
...
//rosrun drone_control drone_control_node quadrotor _cmd_vel:=/quadrotor/cmd_vel
...
//rostopic echo /front_cam/camera/image
//rostopic echo /scan
//rosrun rviz rviz