/**
 * @file MovingObjectVideo.cpp
 * @brief Implementation of video motion detection
 */
...
MotionDetector::MotionDetector(int numFeatures, float matchingThreshold, int minInliers)
    : numFeatures_(numFeatures), matchingThreshold_(matchingThreshold), minInliers_(minInliers) {}

void MotionDetector::UpdateParameters(int nf, float mt, int mi) {
    std::lock_guard<std::mutex> lock(detectorMutex_);
    numFeatures_ = nf; matchingThreshold_ = mt; minInliers_ = mi;
}
...
MotionDetector::CalculateVelocities(const std::vector<cv::Point2f>& points1,
                                     ...
                                     float timeInterval) {
    if (timeInterval <= 0) throw std::invalid_argument("Time interval must be positive");
    if (points1.size() != points2.size()) throw std::invalid_argument("Point array size mismatch");
    ...
    for (size_t i = 0; i < points1.size(); ++i) {
        ...
        velocities.emplace_back(static_cast<float>(cv::norm(vel)), vel);
    }
    return velocities;
}
...
cv::Mat MotionDetector::DetectMotion(const cv::Mat& img1, const cv::Mat& img2,
    ...
    if (img1.empty() || img2.empty()) throw std::invalid_argument("Empty image");
    if (img1.size() != img2.size()) throw std::invalid_argument("Size mismatch");
    if (timeInterval <= 0) throw std::invalid_argument("Non-positive time");
    ...
    } else { gray1 = img1.clone(); gray2 = img2.clone(); }
    ...
    titleStream << std::fixed << std::setprecision(3)
                << "Motion detection - dt: " << timeInterval << "s";
    cv::putText(result, titleStream.str(), cv::Point(20, 30),
    ...
    if (goodMatches.size() >= 4) {
        ...
        if (!H.empty() && cv::countNonZero(inliersMask) >= minInliers_) {
            cv::Mat warped, diff, diffGray;
            cv::warpPerspective(img1, warped, H, img1.size());
            cv::absdiff(warped, img2, diff);
            if (diff.channels() == 3) cv::cvtColor(diff, diffGray, cv::COLOR_BGR2GRAY);
            else diffGray = diff;
            ...
            for (const auto& contour : contours) {
                ...
                float totalVel = 0; int count = 0;
                for (size_t i = 0; i < goodMatches.size(); ++i)
                    if (bbox.contains(pts2[i])) { totalVel += velocities[i].first; ++count; }
                if (count > 0) {
                    std::stringstream ss;
                    ss << std::fixed << std::setprecision(1) << (totalVel / count) << " px/s";
                    cv::putText(result, ss.str(), cv::Point(bbox.x, bbox.y - 10),
