...
istream& operator>> (istream& in, Polynomial& polyn) 
{
    ...
    while (true) 
    {
        ...
        int i = 0;
        for (i = 0; isalnum(ch); i++) // isalnum= Checks whether c is either a decimal digit or an uppercase or lowercase letter.
        { 
            id = ch; // get a variable name
            ch = in.get();
            if (isdigit(ch))// Checks whether c is a decimal digit character. and an exponent (if any); 
            { 
                in.putback(ch);
                in >> exp >> ch;
            }
        ...
        if (ch == ';') // finish if a semicolon is entered;
        {
            ...
            { 
                polyn.error("Term is missing"); // e.g., 2x - ; or just ';'
            }
        }
...
ostream& operator<< (ostream& out, const Polynomial& polyn) 
{
    ...
    for (auto pol = polyn.terms.begin(); pol != polyn.terms.end(); pol++) 
    {
        out.put(' ');
